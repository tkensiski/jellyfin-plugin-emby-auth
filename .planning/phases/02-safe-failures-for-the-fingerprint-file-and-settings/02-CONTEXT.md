...
### New finding — needs a decision outside this phase

**A manual flip to the Default login method can leave an open account, and nothing guards it today.** If an administrator changes `AuthenticationProviderId` to Default for a user on the Emby login method who has no password, that account is then opened by a blank password (`DefaultAuthenticationProvider.cs:62`, confirmed in v12.1). The fingerprint file does not cover this path — it gates only the plugin's own moves. No current requirement covers it, and the deferred item above shows the plugin cannot refuse the change. The options are to detect and compensate after the fact, or to document the hazard. Raise this at the next roadmap review; it is not Phase 2 work.
