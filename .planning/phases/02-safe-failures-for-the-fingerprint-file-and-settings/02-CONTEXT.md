...
- **D-01:** `EmbyVerifiedPasswords` keeps its JSON file in `PluginConfigurationsPath` (`PluginServiceRegistrator.cs:24`, `:34-36`). The maintainer challenged the file three times and every replacement was rejected on evidence:
  ...
  - **Leave the password empty until Emby verifies one**, so that a non-null password means Emby verified it (challenged 2026-09-18). The plugin cannot establish that invariant. An administrator sets a password while the account is still on another login method and only then assigns the Emby login method, which is what `e2e/helpers.bash:133-139` does; no plugin code runs in that order, so the account arrives with a password Emby never verified. Refusing the password set does not close it either: `ChangePassword` only fires while the account is already on the Emby login method, and refusing there would also remove the deliberate move-to-Default path at `EmbyAuthenticationProvider.cs:130-131`. A refusal is technically clean — `UserManager.cs:443-444` at Jellyfin `v12.1` calls the provider before its single `SaveChangesAsync`, so nothing is written — but the administrator sees "Invalid username or password" whatever is thrown, because `ExceptionMiddleware` replaces the message with `"Error processing request."` outside Development and `UserPasswordForm.tsx:100-113` ignores the status and body.
  ...
  - A manual login-method switch cannot be gated at all, which is why the record has to exist before the switch. `UserManager.UpdatePolicyAsync` commits at `:921` and publishes `UserUpdatedEventArgs` at `:925`, after the write, and `EventManager.cs:43-63` logs and swallows anything a consumer throws. `UpdateUserAsync` publishes no event at all. Verified against Jellyfin `v12.1`.
