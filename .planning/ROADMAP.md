...
This roadmap takes the working plugin to a public first release. The security fixes come first: account creation that leaves no open account, and logins that only Emby decides. Next, the fingerprint file and the settings page stop losing data when a read or a request fails. Then the migration status and the migration target, and after them the fingerprint store moves to SQLite and the Emby traffic gets its fixes and tests. The last two phases make the repository safe to publish, then publish one manifest and version 1.0.0.0 so that any Jellyfin 12.1 administrator can install and update the plugin from the catalog.
...
- [x] 04-01-PLAN.md — Tracer: the fingerprint store becomes SQLite, proven by one record written and read back inside the running plugin
- [x] 04-02-PLAN.md — AUTH-05: every access token the plugin can read ends its Emby session
- [x] 04-03-PLAN.md — PERF-01: concurrent readers on an expired user list snapshot send one Emby request
- [x] 04-04-PLAN.md — TEST-05's unit half: the race-loser path, and the reworded account-creation message

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 04-05-PLAN.md — FPRT-04: the legacy fingerprint file is imported once, and never again
- [x] 04-06-PLAN.md — TEST-05 and TEST-06 end to end: concurrent first logins, and invalid settings on a running server

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 04-07-PLAN.md — The JSON-to-SQLite upgrade proven against the real server
...
- [x] 06-02-PLAN.md — PUB-02: the rebuild from the Releases API, the post-deploy verify, the `manifest` mise task, and the separate Pages workflow
- [x] 06-03-PLAN.md — DOCS-02 and DOCS-04: the manifest URL and the catalog install and update steps in the README, and the completed Jellyfin version bump pin list
...
| 6. Catalog Install and First Public Release | 3/5 | In Progress|  |
