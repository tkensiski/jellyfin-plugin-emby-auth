...
4. Choose the **Migration behavior** and the **Account access**, or keep the defaults. Select **Save**.

[Settings](docs/settings.md) explains each choice and how to set the settings through the API.

## Migrate users

1. Ask your users to log in to Jellyfin with their Emby user name and password. The first login creates the Jellyfin account if it does not exist, and saves the password. With the default migration behavior, the user also moves to the Default login method at that login.
2. In the **Migration** section of the settings page, select **Run migration now**. This moves each user marked **ready**.
